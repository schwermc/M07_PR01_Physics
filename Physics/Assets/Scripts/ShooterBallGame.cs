...
public class ShooterBallGame : MonoBehaviour
{
    ...
    void Start()
    {
        if (shootAt == null) { shootAt = Camera.main.transform; }

        soundEffect = GetComponent<AudioSource>();
        if (soundEffect == null) { Debug.LogError("Requires AudioSource component"); }

        pool = GetComponent<ObjectPooler>();
        if (pool == null) { Debug.LogError("Requires ObjectPooler component"); }
    ...
    void ShootBall(Transform shooter)
    {
